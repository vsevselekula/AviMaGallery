...
export function useCampaignFilters(campaigns: Campaign[]) {
  ...
  const searchInCampaign = useCallback((campaign: Campaign, query: string): boolean => {
  ...
  const filteredCampaigns = useMemo(() => {
    ...
    return filtered;
  }, [campaigns, searchQuery, selectedVerticals, selectedTypes, searchInCampaign]);
  ...
  const hasActiveFilters = Boolean(searchQuery.trim()) || selectedVerticals.length > 0 || selectedTypes.length > 0;
