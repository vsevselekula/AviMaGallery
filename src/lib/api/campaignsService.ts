...
export const campaignsService = {
  ...
  async getByVertical(vertical: string): Promise<Campaign[]> {
    ...
    if (error) {
      throw new Error(`Failed to fetch campaigns by vertical: ${error.message}`);
    }
  ...
  async create(campaign: CampaignInsert): Promise<Campaign> {
    ...
    const { id, created_at, updated_at, ...insertData } = campaign as CampaignInsert & { 
      id?: string; 
      created_at?: string; 
      updated_at?: string; 
    };
  ...
  async update(id: string, updates: CampaignUpdate): Promise<Campaign> {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const { created_at, updated_at, ...updateData } = updates as CampaignUpdate & {
      created_at?: string;
      updated_at?: string;
    };
