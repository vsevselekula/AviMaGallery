...
import { uploadCampaignImage, updateCampaignImage, isSupabaseStorageUrl } from '@/lib/storageUtils';
import { isImageFile, formatFileSize, getImageDimensions } from '@/lib/imageUtils';
...
export function ImageUpload({
  ...
  const processImage = useCallback(async (file: File) => {
    ...
    try {
      ...
      const result = value && isSupabaseStorageUrl(value)
      ...
      >
        ...
        {isProcessing ? (
          <div className="flex flex-col items-center space-y-2">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">Загрузка в хранилище...</p>
          </div>
        ) : previewUrl ? (
          <div className="space-y-4">
            ...
            {fileInfo && (
              <div className="text-xs text-gray-500 dark:text-gray-400 space-y-1">
                <div>Размеры: {fileInfo.dimensions.width} × {fileInfo.dimensions.height} px</div>
                <div>Исходный размер: {formatFileSize(fileInfo.originalSize)}</div>
            ...
            <p className="text-xs text-blue-600 dark:text-blue-400 font-medium">
              💡 Совет: можно вставить новое изображение из буфера обмена (Ctrl+V)
            </p>
          </div>
        ) : (
