...
export function Profile2FASettings() {
  ...
  const handleVerificationSubmit = async () => {
    ...
    try {
      ...
      const { data } = await supabase.auth.mfa.listFactors();
      const activeFactor = data?.totp?.find(
        (factor) => factor.status === 'verified'
      );
        ...
        <div className="space-y-4">
          <div className="bg-yellow-900/50 p-4 rounded border border-yellow-500">
            <p className="text-yellow-200 text-sm">
              🔒 Для отключения двухфакторной аутентификации требуется
              подтверждение. Введите код из вашего приложения-аутентификатора:
            </p>
          </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between p-4 bg-gray-700 rounded-lg">
          <div className="flex items-center space-x-3">
            <div
              className={`w-3 h-3 rounded-full ${mfaEnabled ? 'bg-green-500' : 'bg-red-500'}`}
            />
            <div>
