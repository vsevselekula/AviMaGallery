...
export default function Test2FAPage() {
  ...
  const cleanupProblematicFactors = async () => {
    ...
    try {
      ...
      const problematicFactors: Array<{
        id: string;
        type: string;
        name?: string;
        status: string;
      ...
      for (const factorType of allFactorTypes) {
        const factors =
          ((allFactors as Record<string, unknown>)[factorType] as Array<{
            id: string;
      ...
      deleteResults.push(
        `🚨 Найдено ${problematicFactors.length} проблемных факторов:`
      );
      problematicFactors.forEach((factor, index) => {
        deleteResults.push(
          `  ${index + 1}. ID: ${factor.id}, Тип: ${factor.type}, Имя: "${factor.name}", Статус: ${factor.status}`
        );
      });

      if (
        window.confirm(
          `Найдено ${problematicFactors.length} проблемных факторов. Удалить их?`
        )
      ) {
        ...
        for (const factor of problematicFactors) {
          try {
            ...
            if (error) {
              deleteResults.push(
                `❌ Ошибка удаления ${factor.id}: ${error.message}`
              );
            } else {
            ...
            deleteResults.push(
              `❌ Исключение при удалении ${factor.id}: ${delError}`
            );
          }
        }
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          'status' in enrollError &&
          (enrollError as Error & { status: number }).status === 422
        ) {
